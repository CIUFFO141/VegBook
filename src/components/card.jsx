        ...
        <div className="flex justify-center flex-wrap gap-2 p-5">
            ...
            {error &&
                <div className="p-4 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                    <span className="font-medium">An error occurred! Try again</span> {error}
                </div>}
            {recipes.map(recipe => (
                <Link to={`/recipe/${recipe.id}`} key={recipe.id}>
                    <button className="flex justify-center items-center shadow-xl rounded-xl">
                        <div className="group bg-slate-100 w-52 flex flex-col rounded-lg transition ease-in-out lg:hover:scale-110 duration-300 lg:hover:shadow-lg p-2">
                            <div>
                                <img className="rounded-t-xl" src={recipe.image ? recipe.image : Img404} alt={recipe.title} />
                            </div>
